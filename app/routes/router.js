// ============================================================
// routes/router.js
// Router principal — mapeia rotas e chama Controllers
...
// ── Middleware: redireciona profissional para sua área ───────
function verificarUsuario(req, res, next) {
  if (!req.session || !req.session.usuario) return res.redirect("/login");
  if (req.session.usuario.tipo === "profissional") return res.redirect("/profissional/painel-financeiro");
  return next();
}
...
/* ── ROTAS PRIVADAS (usuário cliente) ───────────────────── */
